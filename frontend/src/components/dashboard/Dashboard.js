  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Helper function to dynamically determine the current fiscal year (e.g., July 1 - June 30)
        const getCurrentFiscalYear = () => {
          const now = new Date();
          const currentYear = now.getFullYear();
          const fiscalYearStartMonth = 6; // July is month 6 (0-indexed)
        ...
        // Check if user needs to declare income for the current fiscal year
        if (!taxHistory.some(payment => payment.fiscalYear === currentFiscalYear && payment.status === 'completed')) {
          pendingActions.push({
            id: 'pa1',
            type: 'income_declaration',
            message: `You need to declare your income for the current fiscal year (${currentFiscalYear})`,
        ...
        const lastPayment = taxHistory
          .filter(payment => payment.status === 'completed')
          .sort((a, b) => new Date(b.createdAt || b.date) - new Date(a.createdAt || a.date))[0]?.createdAt || null;

        // Determine tax status based on payments for the current fiscal year
        ...
        if (currentYearPayments.length > 0) {
          // NOTE: A real application would compare the sum of payments against the required tax amount.
          // For this example, we'll assume any payment means it's partially or fully paid.
    ...
    <Box sx={{
      ...
      <Container maxWidth="lg">
        <Box my={4}>
          ...
          {error && (
            <Box my={2}>
              <Alert severity="error" sx={{ bgcolor: 'rgba(255, 0, 0, 0.2)', color: 'white' }}>
                {error}
              </Alert>
            </Box>
          )}
